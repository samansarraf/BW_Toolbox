function ReportGen2 (KK,FileName)
NofSubj = size(KK,2);
%FileName = 'Summary.xlsx';
for iii = 1 : NofSubj
    S = KK{iii}; 
    Subject{1} = ['Subject_' num2str(iii)];
...
%     xlswrite(FileName,M,page,location2)
    location1 = ['A' num2str(s1)];
    xlswrite(FileName,Subject,page,location1);
    l4 = s1+1;
    empty1 = {[],[],[],[]};
    Summary_Name1 = {};
    ...
    for i = 1 : NofModule
        ...
        SizeRoi = [S.J(i)-1 (max(max(S.M)))-S.J(i)];
        ...
        %Summary = [Summary mean(W(:,1)) mean(W(:,2)) mean(S.MeanWithin{1,s}) mean(S.MeanBetween{1,s}) mean(S.MeanEachRegion{1,s})];
